    ...
    <div className={styles.toolbar}>
      ...
      <div className={styles.topRow}>
        <div className={styles.searchWrapper}>
          ...
          />
          {filters.search && (
            <button className={styles.clearSearch} onClick={() => onFilterChange?.('search', '')} aria-label="Clear search">
              ×
            </button>
          )}
        </div>
        ...
        {activeCount > 0 && (
          <button className={styles.clearBtn} onClick={onClear} id="filter-clear" aria-label="Clear all filters">
            Clear all
          </button>
        )}
      </div>
      ...
      {expanded && (
        <div id="filter-advanced" className={styles.advancedRow}>
          ...
          <div className={styles.field}>
            ...
            <select id="filter-sort" className={styles.select} value={filters.sort ?? ''} onChange={set('sort')}>
          ...
          {/* Difficulty */}
          <div className={styles.field}>
            <label className={styles.label} htmlFor="filter-difficulty">Difficulty</label>
            <select id="filter-difficulty" className={styles.select} value={filters.difficulty ?? ''} onChange={set('difficulty')}>
          ...
          {companies.length > 0 && filters.company && (
            <div className={styles.field}>
              <label className={styles.label} htmlFor="filter-time-period">Time Period</label>
              <select id="filter-time-period" className={styles.select} value={filters.timePeriod ?? ''} onChange={set('timePeriod')}>
          ...
          {/* Date range */}
          <div className={styles.field}>
            <label className={styles.label} htmlFor="filter-from">From</label>
            <input id="filter-from" type="date" value={filters.fromDate ?? ''} onChange={set('fromDate')} className={styles.select} />
          </div>
          <div className={styles.field}>
            <label className={styles.label} htmlFor="filter-to">To</label>
            <input id="filter-to" type="date" value={filters.toDate ?? ''} onChange={set('toDate')} className={styles.select} />
          </div>
        </div>
      )}
    </div>

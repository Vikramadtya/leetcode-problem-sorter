...
export default function ReflectionModal({ question, patterns = [], onClose, onSave }) {
  ...
  const [pattern, setPattern] = useState(question.pattern || '');
  const [memoryStrength, setMemoryStrength] = useState(3); // Default to Strong Recall (3)
    ...
    <div className={styles.modalOverlay}>
      <div className={styles.modalContent}>
        <div className={styles.modalHeader}>
          <h3>Reflection Journal: {question.Title}</h3>
        </div>

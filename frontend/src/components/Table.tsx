    ...
    <div className={styles.tableContainer} role="region" aria-label="Questions table">
      <table className={styles.table}>
        <thead>
          <tr>
            {authEnabled && (
              <th className={`${styles.th} ${styles.thCenter}`} scope="col">
                STATUS
              </th>
            )}
            {authEnabled && (
            ...
            {isVisible('ID') && (
              <th
                className={`${styles.th} ${styles.thLeft} ${styles.sortable}`}
            ...
            {!isCompactMode && isVisible('Acceptance %') && (
              <th
                ...
                onClick={() => requestSort('acceptanceRate')}
                scope="col"
              >
            ...
            {!isCompactMode && authEnabled && isVisible('DateSolved') && (
              <th
                className={`${styles.th} ${styles.thCenter} ${styles.sortable}`}
                onClick={() => requestSort('dateSolved')}
                scope="col"
              >
          ...
          {questions.map((q) => {
            ...
            return (
              <tr key={q.id} className={rowClass}>
                  ...
                  <td className={`${styles.td} ${styles.tdCenter}`}>
                    ...
                    >
                      <div className={styles.checkboxRow}>
                        ...
                        >
                          {isAttempted && <span className={styles.attemptedLabel}>A</span>}
                        </button>
                    ...
                    <div className={styles.reviseCell}>
                      ...
                      {prog.nextRevisionDate &&
                        (() => {
                          ...
                          if (diffDays < 0) {
                            badgeClass = styles.srsOverdue;
                            text = `${Math.abs(diffDays)}d OVERDUE`;
                          } else if (diffDays === 0) {
                            badgeClass = styles.srsToday;
                            text = 'TODAY';
                          } else if (diffDays === 1) {
                            text = 'IN 1d';
                          }
                ...
                {/* ID */}
                {isVisible('ID') && (
                  <td className={`${styles.td} ${styles.tdLeft} ${styles.idText}`}>
                    {q.platformId || q.id}
                  </td>
                )}
                      ...
                      {authEnabled && (
                        ...
                        >
                          <svg
                            ...
                            height="16"
                            viewBox="0 0 24 24"
                            fill={prog.important ? '#facc15' : 'none'}
                            stroke={prog.important ? '#facc15' : 'currentColor'}
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            aria-hidden="true"
                    ...
                    <div className={styles.actionRow}>
                      ...
                      {authEnabled && (
                        ...
                        >
                          ...
                          {q.commentsCount > 0 && (
                            <span style={{ marginLeft: '4px' }}>{q.commentsCount}</span>
                          )}
                        </button>
                      )}
                    </div>
                      ...
                      <div className={styles.companiesList}>
                        ...
                        {q.companies.length > 3 && (
                          <span
                            ...
                            title={q.companies.slice(3).map(capitalizeSlug).join(', ')}
                          >
                            +{q.companies.length - 3}
                          </span>
                        )}
                    ...
                    {attempts > 0 || isAttempted ? (
                      <div className={styles.attemptControl}>
                        <button
                          ...
                          onClick={() =>
                            updateProgress(q.id, { attempts: Math.max(0, attempts - 1) })
                          }
                  ...
                  <td className={`${styles.td} ${styles.tdCenter} ${styles.dateText}`}>
                    {prog.dateSolved ? (
                      ...
                      >
                        ...
                        {new Date(prog.dateSolved).toLocaleDateString()}
                      </span>
                    ) : (
                      '-'
                    )}

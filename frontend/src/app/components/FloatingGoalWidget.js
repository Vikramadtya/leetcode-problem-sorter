    ...
    <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', zIndex: 1000, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '1rem' }}>
      ...
      {isOpen && (
        <div style={{
          ...
          <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.1rem', color: 'var(--text-main)', display: 'flex', justifyContent: 'space-between' }}>
            <span>Goals</span>
            <button onClick={() => setIsOpen(false)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 0 }}>✕</button>
          </h3>
          
          <div style={{ marginBottom: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', marginBottom: '4px' }}>
              <span>{getEmoji(dailyPercent)} Daily</span>
              <span style={{ fontWeight: 600 }}>{dailyCount} / {dailyGoal}</span>
            </div>
            <div style={{ height: '8px', background: 'var(--bg-hover)', borderRadius: '4px', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${dailyPercent}%`, background: 'var(--primary)', transition: 'width 0.3s ease' }} />
            </div>
          </div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', marginBottom: '4px' }}>
              ...
              <span style={{ fontWeight: 600 }}>{weeklyCount} / {weeklyGoal}</span>
            </div>
            <div style={{ height: '8px', background: 'var(--bg-hover)', borderRadius: '4px', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${weeklyPercent}%`, background: 'var(--success)', transition: 'width 0.3s ease' }} />
            </div>
          </div>
        </div>
      )}
      ...
      <button 
        ...
        onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.05)'; e.currentTarget.style.boxShadow = '0 6px 16px rgba(0,0,0,0.15)'; }}
